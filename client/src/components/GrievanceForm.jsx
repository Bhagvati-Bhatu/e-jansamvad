...
const GrievanceForm = () => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const spamResp = await fetch("https://e-jansamvad-1.onrender.com/grievance/spam-check", {
        method: "POST",
        headers: {
      ...
      if (file) {
        ...
        const uploadResp = await fetch("https://e-jansamvad-1.onrender.com/upload", {
          method: "POST",
          body: form,
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
      ...
      // ✅ Stop loading after everything completes
      setIsLoading(false);
    }
  }
        ...
        >
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              disabled={isLoading}
              className="w-[23vw] bg-gradient-to-r from-[#ffb703] to-[#fb8500] text-white px-6 py-3 rounded-full font-semibold shadow-md hover:shadow-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isLoading ? (
                <>
